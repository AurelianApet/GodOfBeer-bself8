...
public class MediaPlayerCtrl : MonoBehaviour
{
	...
	Call_Load(strName,0);
        ...
        else
        {
            ...
            if (m_bFirst == false)
            {
                Debug.Log("call load video" + m_strFileName);
                string strName = m_strFileName.Trim();
                Call_Load(strName, 0);
            }
	...
	{
        Debug.Log("load video:" + strFileName);
        if (m_bInit == false)
        {
            InitPlayer();
        }
        ...
        Debug.Log("call_load:" + strFileName);
	...
	private  bool Call_Load(string strFileName, int iSeek)
	{
        ...
        Debug.Log("call video:" + m_video.url);
	...
	IEnumerator CopyStreamingAssetVideoAndLoad(string strURL)
	{
		...
		{
            Debug.Log("load video file:" + strURL);
			Load("file://" + write_path);
		}
	}
